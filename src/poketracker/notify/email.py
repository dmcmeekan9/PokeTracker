...
class SesNotifier:
    def __init__(
        ...
        self.footer_gif_url = os.environ.get("EMAIL_FOOTER_GIF_URL")
...
def render_decision_email(
    ...
    subject = f"{prefix}PokeTracker {decision.type.value}: {decision.item.name}"
...
def _render_html_decision(decision: Decision, footer_gif_url: str | None = None) -> str:
    ...
    footer_gif = ""
    if footer_gif_url:
        escaped_url = html.escape(footer_gif_url, quote=True)
        footer_gif = f"""
        <div style="padding:18px 0 2px;text-align:center;">
          <img src="{escaped_url}" alt="PokeTracker" style="max-width:260px;width:100%;height:auto;border:0;border-radius:8px;" />
        </div>
        """
